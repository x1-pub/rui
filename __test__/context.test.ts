...
import type { HttpContext } from '../src/type.js'
...
describe('上下文模块测试', () => {
  ...
  describe('响应数据设置', () => {
    test('应该正确设置响应数据', () => {
      ...
      expect(result).toBe(ctx)
      expect(ctx.responseData).toBe(data)
    })
    ...
    test('应该支持不同类型的响应数据', () => {
      ...
      ctx.send('string')
      expect(ctx.responseData).toBe('string')
      
      ctx.send(123)
      expect(ctx.responseData).toBe(123)
      
      ctx.send(null)
      expect(ctx.responseData).toBe(null)
      
      const buffer = Buffer.from('test')
      ctx.send(buffer)
      expect(ctx.responseData).toBe(buffer)
    })
  })
  ...
  describe('便利方法', () => {
    ...
    test('应该正确发送文本响应', () => {
      ...
      expect(ctx.responseData).toBe(text)
    })
    ...
    test('应该正确发送 HTML 响应', () => {
      ...
      expect(ctx.responseData).toBe(html)
    })
  ...
  describe('边界情况', () => {
    test('应该处理响应数据为 undefined', () => {
      ...
      ctx.send(undefined)
      expect(ctx.responseData).toBeUndefined()
    })
    ...
    test('应该处理复杂的嵌套对象', () => {
      ...
      expect(ctx.responseData).toBe(complexData)
    })
