...
import type { Http2sRequest, Http2sResponse, Http2sAppOptions, Http2sContext, Next, Http2sMiddleware } from '../type.js'
 ...
 * HTTP/2 安全服务器应用类
 */
class Http2sApp extends ServerFactory<Http2sRequest, Http2sResponse, Http2SecureServer, Http2sAppOptions> {
  ...
  secureServerPush = (resources: Array<{ path: string; headers?: Record<string, string> }>) => {
    return async (ctx: Http2sContext, next: Next) => {
      // HTTP/2 安全服务器推送功能
  ...
  enhancedSecurity = (options: {
    ...
    return (ctx: Http2sContext, next: Next) => {
      ctx.setHeaders({
